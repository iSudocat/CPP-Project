...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		MoveWindow(hwnd, 600, 200, CELL * COLS + 20, CELL * ROWS + 40, FALSE); 
		srand(time(NULL));
		Tetris.ExportBlock();
...
void Game::DoRedirection(HDC hdc)   //改变方向
{
	...
	//temp = (byte*)malloc(sizeof(byte) * cur_block.GetWidth () * cur_block.GetHeight ());
	//std::cout << std::to_string(block_new[i][j]) << std::endl;
	...
	{
		Tetris.DeleteBlock(block_temp);  //退出前必须先释放内存
		return;
	}
	for (i = 0; i < max_len; i++)
	{
	...
	Tetris.DeleteBlock(block_temp);  //释放为临时变量分配的内存
	RefreshPanel(hdc);
}
